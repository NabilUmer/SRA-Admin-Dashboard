...
function App() {
  ...
  const exportToCSV = () => {
    ...
    const headers = ["ID", "Resource", "Time", "Status"];
    const rows = bookings.map(b => {
      const resName = resources.find(r => r.id === b.resourceId)?.name || "Node";
      return [`ALLOC-${b.id}`, resName, new Date(b.startTime).toLocaleString("en-US", {timeZone: "America/Los_Angeles"}), "CONFIRMED"];
    });
  ...
  return (
    <div style={{ margin: 0, padding: '40px', fontFamily: 'sans-serif', backgroundColor: '#0f172a', minHeight: '100vh', width: '100vw', position: 'absolute', left: 0, top: 0, boxSizing: 'border-box', color: '#f8fafc' }}>
      <header style={{ maxWidth: '1200px', margin: '0 auto 40px auto', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', borderBottom: '1px solid #1e293b', paddingBottom: '25px' }}>
        <div>
          <h1 style={{ color: '#60a5fa', margin: 0, fontSize: '3rem', fontWeight: '900' }}>SRA Admin Dashboard</h1>
      ...
      <section style={{ maxWidth: '1200px', margin: '0 auto 40px auto', background: '#1e293b', padding: '35px', borderRadius: '24px' }}>
        <form onSubmit={handleDeploy} style={{ display: 'flex', gap: '20px' }}>
          <input type="text" placeholder="GPU Name" value={newName} onChange={e => setNewName(e.target.value)} style={{ padding: '18px', flex: 2, borderRadius: '14px', border: 'none', background: '#0f172a', color: 'white' }} required />
